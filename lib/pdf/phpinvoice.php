...
use Exception;
use DateTimeZone;
use FPDF\Inc\Rotation\FPDFRotation;
...
class PhpInvoice extends FPDFRotation
{
    ...
    public $curreny_direction = "right";
    ...
    private $columnOpacity   = 0.05;
    private $columnSpacing   = 0.2;
    private $referenceformat = array('.', ',');
    private $margins          = array('l' => 15, 't' => 5, 'r' => 15);

    /******************************************
     * Class Constructor               		 *
     * param : Page Size , Currency, Language *
     ******************************************/
    public function __construct($size = 'A4', $currency = 'FCFA', $language = 'fr')
    {
        $this->columns                = 4;
        $this->items               = array();
        $this->totals              = array();
    ...
    public function addItem($item, $description = "", $quantity = 1, $vat = 0, $price = 0, $discount = 0,$total = 0)
    {
    ...
    public function Header()
    {
        ...
        $positionX = $this->document['w'] - $this->margins['l'] - $this->margins['r'] - max(
            strtoupper($this->GetStringWidth($this->lang['number'])),
            strtoupper($this->GetStringWidth($this->lang['date'])),
            strtoupper($this->GetStringWidth($this->lang['due']))
        ) - 35;
        ...
        //First page
        if ($this->PageNo() == 1) {
            if (isset($this->dimension[1]) && ($this->margins['t'] + $this->dimensions[1]) > $this->GetY()) {
